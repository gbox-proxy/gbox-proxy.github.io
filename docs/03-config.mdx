...
You can config the GBox via environments variables.
...
| GBOX_CACHING_RULES                     | Set of [caching rules](./04-caching/concepts/01-rules.mdx) to control cache behavior for every single query.                                         | [See here](./caching/rules.mdx#default-caching-rules                                       |
| GBOX_CACHING_VARIES                    | Set of [caching varies](./04-caching/concepts/02-varies.mdx) headers and/or cookies to identifier cached query results.                              | ""                                                                                         |
| GBOX_CACHING_TYPE_KEYS                 | [Caching type keys](./04-caching/concepts/03-type-keys.mdx) use to identifier query results items.                                                   | If not specific key field for type, by default "id" will be use as key field for all types |
